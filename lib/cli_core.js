  ...
  parser.getOptFn = function(opt){
    ...
    case "--filter"://指定显示的字段
      fn = function(filter){
        if(arguments.length){
          info.quiet = true;
          info.filters = [].slice.call(arguments);
        }
      };
      break;
    case "--help":
...
function apiExc(apiName, apiExtra){
  ...
  process.nextTick(function(){
    ...
    if(apiName == 'status.update' && info.image && query.status){
      ...
      weiboEmitter = updateStatus(info)
    }else{
    ...
    weiboEmitter.on('success', function(reply){
      ...
      if(isLive){
        if(info.filters){
          info.filters.forEach(function(filterStr){
        ...
        takeAccessToken(function(){
          emitter.emit('retry', apiExc(apiStr, apiQs));
        });
      }
    });
  ...
  return emitter.on('pendStart', function(){
    ...
    info.shower.emit('pendEnd', result, replyOrError);
  }).on('data', function(type, reply){
    if(info.quiet && type === 'error') return;
    log('event: data_' + type);
    info.shower.emit('data', type, reply);
  });
}
