  ...
  parser.getOptFn = function(opt){
    ...
    case "--shower":
      fn = function(showerName){
        ...
        return shower = require('../lib/shower').use(showerName);
      };
      break;
    case "--showers":
      fn = function(){
...
function help(cmd){
  ...
  try{
    ...
    switch(cmd){
      ...
      case undefined:
      case "":
        console.log(fs.readFileSync(__dirname + '/../txt/help.md', 'utf8'), shower.color(Object.keys(cmdMap).concat(Object.keys(aliases)).sort().join(', ')).info);
        break;
      case "execute":
        (function(){
          ...
          if(apiStr && (path = twei.getApiItem(apiStr))){
            console.log(tips['loading_sina_api_item'], shower.color(path.path).yellow);
            open(helpPath + path.path);
              
          }else{
            if(apiStr){
              console.log(tips["api_not_support"], shower.color(apiStr).warn);
            }
      ...
      default:
        if(cmdMap[cmd]){
          ...
          console.log(tips['twei_dont_konw'], shower.color(args.join(', ')).warn);
        }
        break;
    }
    
  }catch(e){
    console.log(tips['need_help'], shower.color(cmd).green);
  }
  exit();
}
