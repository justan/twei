var util = require('util')
  ...
  , open = twei_util.open
  , confPath = (process.env.HOME || process.env.USERPROFILE) + '/.tweirc'
  , noop = function(){}
  ;
...
function config (action, key, val){
  ...
  if(!key && /(get|set|remove|rm|remove-section)/.test(action)){
    console.error(color("需要指定一个 key").error);
    exit(2);
  }
  ...
  switch (action){
    case "get":
      ...
      if(typeof val != 'string'){
        console.error("不存在这样一个配置项: %s", color(key).error);
        exit(2);
      }
      console.log(val);
      break;
...
function getAccessTokenUrl(){
  var Config = require('./config').Config;
  return (new Config(confPath)).get('tsina.token_url') || 'http://t.cn/zWKjcla';
}
...
function apiExc(apiName, apiExtra){
  var Config = require('./config').Config
    ...
    , token = config.get('tsina.access_token') || ''
  ...
  process.nextTick(function(){
    var query = require('querystring').decode(apiExtra)
      , proxy = optInfo.proxy ? optInfo.proxy[0] : config.get('proxy')
      ;
    ...
    if(apiName == 'status.update' && optInfo.image && optInfo.image.length && query.status){
      ...
      weiboEmitter = twei.executeApi(apiName, query, {
        method: optInfo.method && optInfo.method[0] || 'get'
      });
    }
    ...
    weiboEmitter.on('success', function(reply){
      ...
      if(isLive){
        if(optInfo.filter){
          ...
          emitter.emit('data', optInfo.promise && optInfo.promise[0] || apiItem.promise, reply);
        }
