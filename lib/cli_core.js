    ...
    while(arg = args.shift()){
      if(arg.indexOf('-')){//command or param
        if(cmd){
          ...
          cmd = arg;//第一个不带 '-' 的就是 twei 命令
          if(originCmd = restoreAlias(cmd)){
            cmd = originCmd.shift();
            args = originCmd.concat(args);
          }
        }
      }else{//option
        opt = arg;
        ...
        if(flagFn){
          if(flagFn.length){//需要参数的
            isbreak = true;
          }
        }else{//选项参数表中没有列出的, 都当做微博 api 参数
          param.push(arg.replace(/^--?/, '') + '=' + encodeURIComponent(args.shift()));
        }
      }
    }
...
function apiExc(apiName, apiExtra){
  ...
  try{
    ...
    emitter.on('success', function(reply){
      ...
      log(reply);
      isLive = checkreply(reply, function(){
        apiExc(apiName, apiExtra);
      });
      ...
      if(isLive){
        log('event: data')
        if(info.filter){
          info.filter.split(/\s+/).forEach(function(filterStr){
            var filterArgs = filterStr.split('.');
            filterArgs.unshift(reply);
            showData.push(filter.apply(null, filterArgs));
          });
          
          function filter(target){
            var argvs = [].slice.call(arguments, 1)
              , newTarget = target[argvs.shift()]
              , ret = []
              ;
            ...
            if(argvs.length){
              ...
              if(newTarget instanceof Array){
                newTarget.forEach(function(item){
                  ret = ret.concat(filter.apply(null, [item].concat(argvs)));
                });
              }else if(typeof newTarget == 'object'){
                argvs.unshift(newTarget);
                ret = ret.concat(filter.apply(null, argvs));
              }
