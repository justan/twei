 ...
 */
function executeApi(cmd, opts){
  var dfd = new promised.Deferred();
  
  if(!api[cmd.group]){
    process.nextTick(function(){
      dfd.reject('no or not support api: ' + cmd.group + '.' + (cmd.name || ''));
    });
  }
  
  var apiItem = cmd.name ? api[cmd.group][cmd.name] : api[cmd.group]
    , apiPath, method
