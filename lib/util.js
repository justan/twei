    ...
    for(var i = 0, l = this.length; i < l; i++){
      ...
      if(this[i] instanceof Array){
        ...
        for(var j = 0, m = this[i].length; j < m; j++){
          this[i][j] += '';
          if(this.colWidths[j]){
            if(this.colWidths[j] <= this[i].length){
              this[i] = this[i].slice(0, this.colWidths[j] - 4) + '...' 
            }
            
            if(this.maxWidth <= this[i].length){
              this[i] = this[i].slice(0, this.maxWidth - 4) + '...' 
            }
          }else{
            this.maxWidths[j] = Math.max(this.maxWidths[j] || 0, this[i][j].length)
          }
        }
      ...
      if(this[i] instanceof Array){
        for(var j = 0, m = this[i].length; j < m; j++){
          strs += this[i][j] + (function(){
            var s = Math.ceil((that.maxWidths[j] + 1) / TABLEN) - Math.floor(that[i][j].length / TABLEN), tab = '';
            while(s--){ tab += '\t' }
            return tab;
          })();
        }
