...
function Base(){
  ...
  this.on('pendEnd', function(res, err){
    clearInterval(this.pendTimer);
    if(!res && err){
      process.nextTick(function(){
        console.error(color(err).error);
      });
    }
  });
...
Base.prototype.rendData = function(type, reply){
  ...
  default:
    type && console.warn(tips.api_no_shower, type);
    console.log(JSON.stringify(reply, function(key, val){return val}, 2));
    break;
  }
};
